...
- Forked from grocio/participants-recruitment-jp & snishiyama/participants-recruitment-jp.
...
- 2026/07/15
  - TYPE 3 に、最大 5 名の実験者と最大 5 室を使った共同募集を追加しました。Form には同じ時間帯を一度だけ表示し、送信時に空いている実験者と `Room N` の pair を仮割当します。`111` は同じ pair を確定し、`222/333` は Calendar event を削除して pair を解放します。TYPE 1/TYPE 2 の基本動作は変更していません。
  - availability 計算を単一 snapshot にまとめ、各実験者の Calendar を募集期間全体について 1 回だけ取得するよう最適化しました。transparent/free event は空き時間として扱い、結果列と Form choices を同じ snapshot から一括更新します。
  - 「空き予定」に管理者向けの `空き実験者（参考）`、`空き実験室数`、`残り枠`、`割当済み` を追加しました。「メンバー」の `担当回数` も、回答表の `仮割当` と `確定` から自動計算します。
  - 参加者向けメールの場所は「設定」の `experimentRoom` を使用し、内部リソース番号の `Room N` は回答表と担当者 Calendar だけに記録するよう修正しました。
  - Form の公開、回答表の列設定、デフォルトの責任者情報、Calendar ID と共有権限、迷惑メール対策など、TYPE 3 の導入・運用上の確認事項を追記しました。
  - `Cached` sheet の欠落・破損時に設定を自動再構築するよう修正し、TYPE 3 初期化時の「空き予定」列数不一致も修正しました。
